...
BOOL kIsOutputBufferFull(){
    // 상태 레지스터에서 읽은 값에 출력 버퍼 상태 비트(0비트)가 1로 설정되어 있으면 출력 버퍼에 키보드가 전송한 데이터 존재
...
// 입력 버퍼에 프로세서가 쓴 데이터가 남아있는지 여부 반환
BOOL kIsInputBufferFull(){
    // 상태 레지스터에서 읽은 값에 입력 버퍼 상태 비트(1비트)가 1로 설정되어 있으면 키보드가 아직 데이터를 가져가지 않았음
...
// ACK를 기다림, ACK가 아닌 다른 스캔 코드는 변환해서 큐에 삽입
BOOL kWaitForACKAndPutOtherScanCode(){
    ...
    // 키보드에 전달된 데이터를 최대 100개까지 수신하여 ACK 확인
    for(j = 0; j < 100; j++){
        for(i = 0; i < 0xFFFF; i++){
...
BOOL kActivateKeyboard(){
    ...
    // 컨트롤 레지스터에 키보드 활성화 커맨드(0xAE)를 전달하여 키보드 디바이스 활성화
    kOutPortByte(0x64, 0xAE);

    // 입력 버퍼가 빌때까지 기다렸다가 키보드에 활성화 커맨드 전송
    for(i = 0; i < 0xFFFF; i++){
...
BOOL kChangeKeyboardLED(BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn){
    ...
    for(i = 0; i < 0xFFFF; i++){
        // 입력 버퍼가 비어 있으면 키보드가 LED 데이터르 가져간 것임
        if(kIsInputBufferFull() == FALSE){
            break;
        }
    }
...
BOOL kIsNumberOrSymbolScanCode(BYTE bScanCode){
    // 숫자 패드나 확장 키 범위를 제외한 범위에서 영문자가 아니면 숫자나 기호임
...
BOOL kIsUseCombinedCode(BYTE bScanCode){
    ...
    if(kIsAlphabetScanCode(bDownScanCode) == TRUE){
        // 만약 Shift키와 CapsLock 키 중에 하나만 눌러져 있으면 조합된 키를 되돌려줌
    ...
    // 0xE0만 제외하면 확장 키 코드와 숫자 패드의 코드가 겹치므로 확장 키 코드가 수신되지 않았을 때만 처리 조합된 코드 사용
    else if((kIsNumberPadScanCode(bDownScanCode) == TRUE) && (gs_stKeyboardManager.bExtendedCodeIn) == FALSE){
        // NumLock 키가 눌러져 있으면 조합된 키를 되돌려 줌
...
void UpdateCombinationKeyStatusAndLED(BYTE bScanCode){
    ...
    // 눌림 또는 떨어짐 상태처리, 최상위 비트(비트7)가 1이면 키가 떨어짐, 0이면 눌림
    if(bScanCode & 0x80){
        bDown = FALSE;
        bDownScanCode = bScanCode & 0x7F;
    }
    ...
    // NumLock 키의 스캔 코드(69)이면 NumLock의 상태를 갱신하고 LED 상태 변경
    else if((bDownScanCode == 69) && (bDown == TRUE)){
        gs_stKeyboardManager.bNumLockOn ^= TRUE;
        bLEDStatusChanged = TRUE;
    }
    // ScrollLock 키의 스캔 코드(70)이면 ScrollLock의 상태를 갱신하고 LED 상태 변경
    else if((bDownScanCode == 70) && (bDown == TRUE)){
        gs_stKeyboardManager.bScrollLockOn ^= TRUE;
        bLEDStatusChanged = TRUE;
    }

    // LED상태가 변했으면 키보드로 커맨드를 전송하여 LED를 변경
...
BOOL kGetKeyFromKeyQueue(KEYDATA* pstData){
    BOOL bResult, bPreviousInterrupt;
