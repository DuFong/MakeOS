...
// /**
//  *  태스크 리스트에서 다음으로 실행할 태스크를 얻음 (Multilevel Queue Scheduler)
//  */
...
static TCB* kGetNextTaskToRun( void )
{
    ...
    for(i = 0; i<TASK_MAXREADYLISTCOUNT; i++){
        iTaskCount = kGetListCount(&(gs_stScheduler.vstReadyList[i]));
        totaltickets += (iTaskCount * getTicket(i));
    }
    
    // Lottery Scheduler를 이용
    //pstTarget = kGetNextTaskToRunbyLottery(totaltickets);
    // Stride Scheduler를 이용
    pstTarget = kGetNextTaskToRunbyStride();

    return pstTarget;
}

// Lottery Scheduler를 이용한 kGetNextTaskToRun
static TCB* kGetNextTaskToRunbyLottery(int totaltickets)
{
    TCB* pstTarget = NULL;
    TCB* current = NULL;
    int iTaskCount, i;
    QWORD counter = 0, winner;
    
    // 랜덤 값 뽑기
    winner = getrandom(totaltickets);

    // task를 돌며 ticket수를 확인하며 counter에 더해주면서 counter>=winner가 되는 task 수행
    for( i = 0 ; i < TASK_MAXREADYLISTCOUNT ; i++ )
    {
        iTaskCount = kGetListCount( &( gs_stScheduler.vstReadyList[ i ] ) );
...
QWORD getrandom(int final)
{
    ...
    return gs_qwRandomValue % (final+1);
}

// Stride Scheduler를 이용한 kGetNextTaskToRun
static TCB* kGetNextTaskToRunbyStride(void)
{
    TCB* pstTarget = NULL;
    TCB* current = NULL;
    int iTaskCount, i;
    QWORD min_pass;
	int list_index;     // min_pass 를 가진 Task의 큐 index 저장을 위해
    BOOL is_first = TRUE;
    
    // 모든 Task를 돌며 가장 작은 pass 값을 가진 Task를 찾는다.  가장 높은 우선순위의 큐 부터 탐색
    for( i = 0 ; i < TASK_MAXREADYLISTCOUNT ; i++ )
    {
        iTaskCount = kGetListCount( &( gs_stScheduler.vstReadyList[ i ] ) );
        if(iTaskCount == 0) continue;
        ...
        // min_pass 와 pstTarget 을 첫번쨰로 검사하는 Task로 초기화
	    if(is_first){
			list_index = i;
	        min_pass = current->pass;
	        pstTarget = current;
	        is_first = FALSE;
		}

        while(current){
            // 현재 min_pass 보다 더 작은 pass값 발견 시 큐 index 와 Task 기억
            if(min_pass > current->pass){
				list_index = i;
                min_pass = current->pass;
                pstTarget = current;
            }
            // 현재 큐의 Tail 부분까지 검사했을 시 while문 탈출
			if(current == kGetTailFromList(&(gs_stScheduler.vstReadyList[i])))
				break;
            current = kGetNextFromList(&( gs_stScheduler.vstReadyList[ i ] ), current);
        }
    }
    if(pstTarget != NULL){
        // stride 만큼 Task의 pass값 증가
        pstTarget->pass = pstTarget->pass + pstTarget->stride;
		pstTarget = kRemoveList(&(gs_stScheduler.vstReadyList[list_index]), pstTarget->stLink.qwID);
    }
    return pstTarget;
}
...
static BOOL kAddTaskToReadyList( TCB* pstTask )
{
    ...
    // ticket 우선순위에 따라 부여
    pstTask->ticket = getTicket(bPriority);
    //부여된 ticket에 따라 stride 부여
    pstTask->stride = (QWORD)STRIDE_OPERAND / pstTask->ticket;
...
BOOL kChangePriority( QWORD qwTaskID, BYTE bPriority )
{
    ...
    if( pstTarget->stLink.qwID == qwTaskID )
    {
        ...
        pstTarget->ticket = getTicket(bPriority);
        pstTarget->stride = (QWORD)STRIDE_OPERAND / pstTarget->ticket;
    }
