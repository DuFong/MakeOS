...
// 태스크를 생성, 태스크 ID에 따라서 스택 풀에서 스택 자동 할당
TCB* kCreateTask(QWORD qwFlags, QWORD qwEntryPointAddress){
    ...
    // TCB를 설정한 후 준비 리스트에 삽입하여 스케쥴링 될 수 있도록 함
...
/* 스케쥴러 관련 */
// 스케쥴러 초기화, TCB 풀과 init task도 같이 초기화
...
// 태스크를 스케쥴러의 준비 리스트에 삽입
void kAddTaskToReadyList(TCB* pstTask){
    kAddListToTail(&(gs_stScheduler.stReadyList), pstTask);
}

// 다른 태스크를 찾아서 전환, 인터럽트나 예외가 발생했을 때 호출하면 안됨
void kSchedule(){
    ...
    // 전환하는 도중 인터럽트가 발생하여 태스크 전환이 또 일어나면 곤란하므로 전환하는 동안 인터럽트 막음
...
// 인터럽트가 발생했을 때, 다른 태스크를 찾아 전환, 반드시 인터럽트나 예외가 발생했을 때 호출
BOOL kScheduleInInterrupt(){
    ...
    // 태스크 전환 처리, 인터럽트 핸들러에서 저장한 콘텍스트를 다른 콘텍스트로 덮어씀
    ...
    // 전환해서 실행할 태스크를 Running Task로 설정하고 콘텍스트를 IST에 복사해서 자동으로 태스크 전환이 이루어짐
    gs_stScheduler.pstRunningTask = pstNextTask;
